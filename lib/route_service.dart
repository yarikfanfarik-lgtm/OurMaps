  ...
  static Future<RouteResult> route({
    ...
    if (mode == RouteMode.transit || mode == RouteMode.metro) {
      return RouteResult(
        ...
        distanceMeters: 0,
        durationSeconds: 0,
        title: mode == RouteMode.metro ? 'Метро' : 'Общественный транспорт',
        steps: const [
          'Для расчёта общественного транспорта нужен отдельный transit-бэкенд. OSM хранит остановки и линии, но сам по себе маршруты не рассчитывает.',
        ],
      );
    }
    ...
    final response = await http.get(
      uri,
      headers: {'User-Agent': 'OurMaps/0.1'},
    );

    if (response.statusCode != 200) {
      throw Exception('Маршрутизатор недоступен');
    }
    ...
    for (final leg in (r['legs'] as List)) {
      for (final s in (leg['steps'] as List)) {
        ...
        if (type == 'depart') {
          steps.add('Начните движение${name.isNotEmpty ? ' по $name' : ''}');
        } else if (type == 'arrive') {
          steps.add('Вы прибыли');
        } else {
          steps.add(
            '${_modifier(mod)}${name.isNotEmpty ? ' на $name' : ''}',
          );
        }
      }
    }
    ...
    return RouteResult(
      ...
      title: mode == RouteMode.walking
          ? 'Пеший маршрут'
          : 'Маршрут на машине',
      steps: steps.take(12).toList(),
    );
  }

  static String _modifier(String m) => switch (m) {
